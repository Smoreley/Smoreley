...
local fs = require("pato.utils.fs")
...
local function path_to_lua_module(module_path)
  local lua_path = string.format("%s%slua", system.pato_root, system.sep)
...
local function get_lua_modules(path)
  -- Look for Lua modules in Pato Nvim root
...
reloader.reload_lua_module = function(mod_path, quiet)
  ...
  -- If pato-nvim fails to reload, warn user once per session
  if mod_path == nil then
    if reloader.has_failed_reload == false then
      log.warn(
        "reloader: Failed to reload pato config because this file is not in nvim config directory.  Is your pato nvim config directory symlinked?"
      )
...
reloader.reload_lua_modules = function(quiet)
  local paths = vim.fn.glob(system.pato_root .. system.sep .. "lua", 0, 1)
...
--- Reload the plugins definitions modules like pato_modules.lua to automatically
--- install or uninstall plugins on changes
reloader.reload_plugins_definitions = function()
  -- Silently reload plugins modules
  reloader.reload_lua_module("pato.core.config.modules", true)
  reloader.reload_lua_module("pato.core.config.userplugins", true)
  reloader.reload_lua_module("pato.modules", true)
...
reloader.full_reload = function()
  -- Store the time taken to reload Pato
  local reload_time = vim.fn.reltime()
  log.info("Reloading Pato ...")
