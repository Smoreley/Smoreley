...
local round = require("pato.utils").round
-- logging defaults to "info" level
local logging_level = "info"

-- Manually load pato config.lua to avoid circular dependencies
local ok, ret = xpcall(require, debug.traceback, "pato_config")
if ok then
    logging_level = ret.config.pato.logging or logging_level
else
    ok, ret = xpcall(dofile, debug.traceback, system.pato_configs_root .. "/pato_config.lua")
    if ok then
        logging_level = ret.config.pato.logging or logging_level
    end
end
...
local default_config = {
    -- Name of the the plugin
    plugin = "pato",
    -- Should print the output to neovim while running
    use_console = true,
    -- Should highlighting be used in console
    highlights = true,
