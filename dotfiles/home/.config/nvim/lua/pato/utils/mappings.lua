...
if mod_utils.is_module_available("nvim_mapper") then
    ...
    mappings.map_buf =
        function(bufnr, mode, lhs, rhs, opts, category, unique_identifier, description)
            mapper.map_buf(bufnr, mode, lhs, rhs, opts, category, unique_identifier, description)
        end
    mappings.map_virtual =
        function(mode, lhs, rhs, opts, category, unique_identifier, description)
    ...
    mappings.map = function(mode, lhs, rhs, opts, _, _, _)
        local options = config.pato.allow_default_keymaps_overriding and {} or { noremap = true }
        if opts then
...
mappings.execute = function(id)
    ...
    if not func then
        require("pato.extras.logging").error("Function doest not exist: " .. id)
    end
    return func()
end
...
local function key_map(mode, key, cmd, opts, defaults)
    ...
    if type(cmd) == "function" then
        ...
        if opts.expr then
            cmd = ([[luaeval('require("pato.utils").execute(%d)')]]):format(#mappings.functions)
        else
            cmd = ("<cmd>lua require('pato.utils').execute(%d)<cr>"):format(#mappings.functions)
        end
    end
